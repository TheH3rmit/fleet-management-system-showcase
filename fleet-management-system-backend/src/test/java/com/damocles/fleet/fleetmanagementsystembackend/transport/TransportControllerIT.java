...
class TransportControllerIT extends AbstractPostgresIT {
    ...
    @Test
    void driver_accepts_transport_admin_cannot_set_driver_statuses_and_delete_after_accept() throws Exception {
        ...
        Driver driver = dataFactory.createDriver(driverAcc.getUser(), "LIC-1");

        var pickup = dataFactory.createLocation("pickup");
        var delivery = dataFactory.createLocation("delivery");
        var vehicle = dataFactory.createVehicle("PL-123");
        var trailer = dataFactory.createTrailer("TR-123");
        ...
        mvc.perform(delete("/api/transports/" + transportId)
                        .header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isConflict());
    }
